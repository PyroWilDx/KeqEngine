...
void Keqing::moveX() {
    if (!isSpriteAnimated(KQ_NATK) &&
        ...
        if (Global::gWorld->isPixelSurface(vX, yDown)) {
            Entity::moveX();
        }
    }
}
    ...
    if (Particle::isActive(PARTICLE_KQ_CATK)) {
        ...
        if (CAtkParticle->isNewestFrame(1)) {
            ...
            shiftXYArray(atkPolyPts, N,
                         20, 0);
            Attack *atk =
                    Global::gWorld->addKQAtk(this, CAtkParticle,
                                             atkPolyPts, N,
                                             1.52, 0.8, -0.2);
...
void Keqing::crouchCAtk() {
    ...
    if (Particle::isActive(PARTICLE_KQ_CROUCH_CATK)) {
        ...
        if (crouchCAtkParticle->isNewestFrame(1)) {
            ...
            Attack *atkLeft =
                    Global::gWorld->addKQAtk(this, atkPolyPtsLeft, HalfPolyN,
                                             1.52, -0.2, 0.6);
            ...
            Attack *atkRight =
                    Global::gWorld->addKQAtk(this, atkPolyPtsRight, HalfPolyN,
                                             1.52, 0.2, 0.6);
            ...
            atkLeft->setAtkCompanion(atkRight);
            ...
            atkLeft->setHitSound("KQHitSwordStrong1.ogg");
            ...
            atkRight->setAtkCompanion(atkLeft);
        }
    }
}
        ...
        if (isNewestFrame(7, KQ_SKILL_FLIP)) {
            ...
            double KqAtkPolyPts[KqN][2] =
                    {{52,  -32},
                     ...
                     {54,  -24}};
            Attack *KqAtk =
                    Global::gWorld->addKQAtk(this, KqAtkPolyPts, KqN,
                                             1.42, -0.82, -0.56);
            ...
            rotateXYArray(PtAtkPolyPts, PtN,
                          ...
                          (0.96 * M_PI) / 4., false);
            Attack *PtAtk =
                    Global::gWorld->addKQAtk(this, flipSlashParticle,
                                             PtAtkPolyPts, PtN,
                                             2.2, 1.06, 0.6);
        ...
        if (!Particle::isActive(PARTICLE_KQ_SKILL_CLONE_SPAWN) &&
            ...
            cloneSpawnParticle->setOnRender([](Particle *particle) {
                if (particle->willFrameFinish(3)) {
                    ...
                    auto atkOnHit =
                            [](Attack *atk, LivingEntity *hitEntity,
                               void *fParams) {
                                auto *cloneIdleParticle = (Particle *) fParams;
                                cloneIdleParticle->removeSelf();
                            };
                    auto atkShouldRemove =
                            [](Attack *atk, void *fParams) {
                                return (!Particle::isActive(
                                        PARTICLE_KQ_SKILL_CLONE_IDLE));
                            };

                    Attack *atkLeft =
                            Global::gWorld->addKQAtk(kq, cloneIdleParticle,
                                                     atkPolyPtsLeft, N,
                                                     1.2, -0.56, -0.26);
                    atkLeft->setClassicParticle(0, true);
                    atkLeft->setOnHit(atkOnHit,
                                      (void *) cloneIdleParticle);
                    atkLeft->setShouldRemove(atkShouldRemove,
                                             nullptr);
                    ...
                    Attack *atkRight =
                            Global::gWorld->addKQAtk(kq, cloneIdleParticle,
                                                     atkPolyPtsRight, N,
                                                     1.2, 0.56, -0.26);
                    atkRight->setClassicParticle(0, true);
                    atkRight->setOnHit(atkOnHit,
                                       (void *) cloneIdleParticle);
...
void Keqing::createSkillSpawnParticle() {
    ...
    spawnParticle->moveToNearestSurface();
...
void Keqing::createLightningStelitto() {
    ...
    Particle *idleParticle =
            Particle::pushParticle(PARTICLE_KQ_SKILL_IDLE,
                                   40, 2., 2.);
    idleParticle->setSpriteFrameLengthFromTo(
            1000 - idleParticle->getSpriteLengthFromTo(1, -1),
            0, 0);
    idleParticle->setParticleEntity(spawnParticle, false,
                                    false);
    idleParticle->moveToEntityCenter(spawnParticle, false);
    idleParticle->setHitBoxCentered((int) spawnParticle->getBaseHitBoxW(),
                                    (int) spawnParticle->getBaseHitBoxH());
...
void Keqing::createSkillProjAfterFx() {
    Particle *projParticle =
            Particle::getParticle(PARTICLE_KQ_SKILL_PROJ);
    Particle *projAftFxParticle =
            Particle::pushParticle(PARTICLE_KQ_SKILL_PROJ_AFT_FX, 20);
    projAftFxParticle->setRotation(projParticle->getRotation());
    projAftFxParticle->moveToEntityCenter(projParticle);
}

void Keqing::finishSkillProjParticle() {
...
void Keqing::createSkillProjParticle() {
    ...
    projParticle->setRotationMovingRot(radToDegree(angle));
    projParticle->moveToEntityCenter(kq);
    projParticle->setHitBoxCentered(spawnParticle->getHitBox().w,
                                    spawnParticle->getHitBox().h);
    projParticle->setXYVelocity(KQ_LIGHTNING_STELITTO_VELOCITY,
                                -KQ_LIGHTNING_STELITTO_VELOCITY);
    ...
    projParticle->setOnRender([](Particle *particle) {
        ...
        if ((lastCenterX < dstX && newCenterX >= dstX) ||
            ...
            (lastCenterY > dstY && newCenterY <= dstY)) {
            Keqing::finishSkillProjParticle();
        }
    }, fParams);
    ...
    Attack *atk =
            Global::gWorld->addKQAtk(kq, projParticle,
                                     atkPolyPts, N,
                                     0.907, kbXV, kbYV);
    ...
    atk->setOnHit([](Attack *atk, LivingEntity *dstEntity,
                     void *fParams) {
        Particle *spawnParticle =
                Particle::getParticle(PARTICLE_KQ_SKILL_SPAWN);
        spawnParticle->setEntity(dstEntity, false,
                                 false);
        Keqing::createSkillProjAfterFx();
        Keqing::finishSkillProjParticle();
    }, nullptr);
...
void pushBurstSlashAtk(double slashDmgPercent, double kbXV, double kbYV,
                       int duration, int typeSlash) {
    // TypeSlash : 0 = Initial Slash
    //             1 = Clone Slash
    //             2 = Slash
    //             -1 = Final Slash
    ...
    Attack *atkLeft =
            Global::gWorld->addKQAtk(kq, atkPolyPtsLeft, HalfPolyN,
                                     slashDmgPercent, -kbXV, kbYV);
    atkLeft->setAtkDuration(duration);
    atkLeft->setClassicParticle(nParticle[typeSlash + 1], true);

    Attack *atkRight =
            Global::gWorld->addKQAtk(kq, atkPolyPtsRight, HalfPolyN,
                                     slashDmgPercent, kbXV, kbYV);
    atkRight->setAtkDuration(duration);
    atkRight->setClassicParticle(nParticle[typeSlash + 1], true);
    ...
    if (typeSlash == 0) {
        ...
        atkRight->setShouldRemove(atkShouldRemove, nullptr);
        atkLeft->setShouldRemove(atkShouldRemove, nullptr);
    }
    ...
    if (typeSlash != -1) {
        Global::gWorld->getBackground()->shakeScreen(4, 4, 200);
    }
}
...
void pushCloneSlashParticle(Particle *removedParticle) {
    ...
    if (cSlashCount < KQ_BURST_NUMBER_OF_CLONE_SLASH) {
        ...
        Global::gWorld->addIgnoreFilterEntity(cSlashParticle);

        kq->setBurstCloneSlashCount(cSlashCount + 1);

        // Push Atk
        pushBurstSlashAtk(0.22, 0.072, -0.1312,
...
void Keqing::RBurst() {
    ...
    if (isNewestFrame(0, KQ_BURST)) { // Burst Start
        // 114, 86, 180
        Global::gWorld->enableColorFilter(74, 46, 140, 60);
        Global::gWorld->addIgnoreFilterEntity(this);
        ...
        Global::gWorld->addIgnoreFilterEntity(aoeParticle);
        ...
        Global::gWorld->addIgnoreFilterEntity(vanishParticle);
        ...
        Global::gWorld->addIgnoreFilterEntity(cloneParticle);
        ...
        Global::gWorld->addIgnoreFilterEntity(finalSlashParticle);
        ...
        for (int i = 0; i < KQ_BURST_NUMBER_OF_CLONE; i++) {
            ...
            Global::gWorld->addIgnoreFilterEntity(cloneVanishParticle);
        }
        ...
        Global::gWorld->getBackground()->shakeScreen(32, 20, 600);

        Sound::playAudioChunk("res/sfx/particle/KQBurstEnd.ogg");

        // Push Atk
        pushBurstSlashAtk(3.4, 1.2, -0.6,
                          80, -1);

    } else if (isNewestFrame(-1, KQ_BURST)) {
        Global::gWorld->disableColorFilter();
    }
    ...
    if (isFrameAt(13, KQ_BURST)) {
        ...
        for (int i = 0; i < KQ_BURST_NUMBER_OF_CLONE; i++) {
            ...
            if (cloneParticle->isNewestFrame(5 * i)) {
                ...
                Global::gWorld->addIgnoreFilterEntity(cloneAppearParticle);
            }
        }
        ...
        if (!Particle::isActive(PARTICLE_KQ_BURST_CLONE_SLASH)) { // Clone Slash End
            ...
            for (int i = 0; i < KQ_BURST_NUMBER_OF_SLASH; i++) {
                ...
                const double addMoveGap = 40.;
                slashParticle->moveAdd(
                        Random::getRandomReal(-addMoveGap, addMoveGap),
                        Random::getRandomReal(-addMoveGap, addMoveGap)
                );
                slashParticle->setRotation(
                        Random::getRandomReal(0, 360.));
                slashParticle->delaySprite(2 * slashDuration * i);
                slashParticle->setOnRender([](Particle *particle) {
                    if (particle->isNewestFrame(0)) {
                        // Push Atk
                        pushBurstSlashAtk(0.432, 0.08, -0.18,
                                          40, 2);
                    }
                });
                Global::gWorld->addIgnoreFilterEntity(slashParticle);
            }
        }
    }
}
...
void Keqing::airPlunge() { // Plunge Attack in Genshin
    ...
    if (isFrameBetween(6, 7, KQ_AIR_PLUNGE)) {
        if (!isInAir()) {
            ...
            clearAndShrinkVector(&airPlungeHitEntityVector);
            airPlungeHitEntityVector.clear();
            airPlungeHitEntityVector.shrink_to_fit();
    ...
    if (!airPlungeHitEntityVector.empty()) {
        double yPlungeDownMax = y + getBaseHitBoxH() - 16.;
        for (LivingEntity *hitEntity: airPlungeHitEntityVector) {
    ...
    if (isNewestFrame(6, KQ_AIR_PLUNGE)) {
        ...
        atk->setOnHit([](Attack *atk, LivingEntity *hitEntity,
            ...
            kq->addAirPlungeHitEntity(hitEntity);
        }, nullptr);
        ...
        double atkPolyPtsLeft[N][2] =
                {{-46, 0},
                 ...
                 {-46, 42}};
        Attack *atkLeft =
                Global::gWorld->addKQAtk(this, atkPolyPtsLeft, N,
                                         2.53, -0.6, -0.4);
        ...
        Attack *atkRight =
                Global::gWorld->addKQAtk(this, atkPolyPtsRight, N,
                                         2.53, 0.6, -0.4);
...
bool Keqing::onDeath() {
    ...
    if (fOnDeathRetryButton != nullptr) {
        tmpRect = {0, 0, 200, 100};
        Button *retryButton = Draw::drawRetryButton(&tmpRect, deathText,
                                                    fOnDeathRetryButton, fParamsRetryButton);
        tmpRect = {0, 0, 200, 100};
        Draw::drawHomeButton(&tmpRect, retryButton, &Global::gInfo->gRunning);
    } else {
        tmpRect = {0, 0, 200, 100};
        Draw::drawHomeButton(&tmpRect, deathText, &Global::gInfo->gRunning);
    }
